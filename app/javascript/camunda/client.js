...
class CamundaClient {
  ...
  async startProcess(ProcessKey, Variables, callback) {
    ...
    await axios.post( this.url + '/engine-rest/process-definition/key/' + ProcessKey + '/start', {variables: Variables, businessKey: this.BusinessKey})
    .then(response => {
     ...
     this.consumer.subscriptions.create({ channel: "Camunda", room: this.businessKey },
     {
       connected() {
